...
`skills/houdini-production-workflows/` is the source-controlled decision and
experience layer above MCP. Preserve its progressive-disclosure structure:

- Keep shared routing and invariants concise in `SKILL.md`.
- Put domain-specific procedures in `references/workflows/` and load only the
  workflow relevant to the task.
- Treat case records as scoped evidence, recipes as executable intent, and
  acceptance checks as the completion authority.
- Keep automatically ingested records at `candidate`; status promotion needs
  reproducible evidence and must not be performed by an ingestion script.
- Validate JSON records with `scripts/experience_cli.py validate` and rebuild
  `references/records/index.json` after record changes.
- Keep the dashboard read-only and generated from validated records. Do not
  encode knowledge directly in its HTML template or generated output.
- Keep its browser runtime self-contained and offline. Pin vendored libraries,
  preserve their license files, and never add runtime telemetry or network I/O.
- Run `scripts/inspect_houdini_case.py` only under Hython. It must remain
  non-cooking and must never save, cache, render, export, or rewrite the source.
- Do not commit machine-specific paths generated by automatic inspection.
- Add a binary HIP/HDA template only with a strict template manifest, safe
  load behavior, a real recipe consumer, and Hython smoke coverage.

When repeated corrections reveal a deterministic cross-workflow operation,
consider a Catalog capability. Keep artistic judgment, project conventions,
and workflow selection in the skill or a project profile.
...
Production knowledge tests:

```powershell
python -m unittest -v tests/test_experience_system.py
python skills/houdini-production-workflows/scripts/experience_cli.py validate
python skills/houdini-production-workflows/scripts/experience_cli.py dashboard
```
